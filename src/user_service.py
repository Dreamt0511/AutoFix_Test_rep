...
def get_user(user_id):
    ...
    try:
        ...
        logger.exception(f"get_user({user_id}) 用户ID格式错误")
        return None
...
def get_user_email(user_id):
    ...
    logger.warning(f"get_user_email({user_id}) 用户不存在")
    return None
...
def create_user(name, email, role):
    ...
    if role not in ("admin", "user"):
        logger.error(f"create_user 非法角色: {role}")
        raise ValueError(f"无效的角色: {role}，必须是 admin 或 user")
    # 校验邮箱唯一性
...
def delete_user(user_id):
    ...
    try:
        ...
        logger.exception(f"delete_user({user_id}) 用户ID格式错误")
        return False
